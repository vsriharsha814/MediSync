...
export default class LLMInterface {
  ...
  private SYSTEM_PROMPT = `You are a specialized medical research expert called 
    MediSync. Provide in-depth, accurate medical answers in clear markdown with 
    bullet points, headings, etc. You will not entertain any questions not related
    to healthcare or medicine`;
  ...
  async send(optimizedQueries: string[]) {
    ...
    if (!response.ok) {
      return this.res.status(response.status).json({
